...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (sendConfirmationNow) {
      // NOT fire-and-forget on purpose: Vercel can freeze/tear down the
      // serverless function the instant the response is sent, which kills
      // any unawaited promise still in flight - that's why this call was
      // never reaching n8n at all (0 executions logged there). Awaiting it
      // adds a bit of latency to the response but guarantees the WhatsApp
      // send actually happens.
      try {
        const confirmRes = await fetch('https://n8n.srv1623198.hstgr.cloud/webhook/impex-complaint-confirm', {
          method: 'POST',
        ...
        if (!confirmRes.ok) {
          console.error('impex-complaint-confirm returned non-OK status', confirmRes.status, await confirmRes.text());
        }
      } catch (e) {
        // Don't fail the whole registration just because the notify call failed -
        // the row is already written, the customer still gets their UID on screen.
