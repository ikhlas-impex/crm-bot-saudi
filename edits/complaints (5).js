      ...
      <table style={{ width: '100%', borderCollapse: 'collapse' }}>
        <thead>
          <tr>
            ...
            <th>Payment Status</th><th>Status</th><th>Review Status</th><th>Service Centre</th>
        ...
        <tbody>
          {complaints.map((c) => (
            <tr key={c.uid || `${c.phone}-${c.createdat}`}>
              ...
              <td>{c.status}</td>
              <td>
                {!c.reviewstatus && <span style={{ color: '#999' }}>Pending</span>}
                {c.reviewstatus === 'CHECKED' && <span style={{ color: '#2e7d32' }}>✓ Checked</span>}
                {c.reviewstatus === 'NOT_CHECKED' && <span style={{ color: '#c62828' }}>✗ Not Checked</span>}
              </td>
              ...
              <td>
                {c.status === 'PENDING_PAYMENT_VERIFICATION' ? (
                  ...
                  <a href={`/admin/followup?uid=${encodeURIComponent(c.uid)}`}>
          ...
          {!loading && complaints.length === 0 && (
            <tr><td colSpan={14} style={{ textAlign: 'center', padding: 16 }}>No complaints found.</td></tr>
          )}
        </tbody>
      </table>
    </div>
